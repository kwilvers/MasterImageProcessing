...
from keras.layers import Dropout        #module de overfitting, supprimer les neurones ayant trop de poids
...
'''Ajouter une couche pour "supprimer l'overfitting"'''
#On l'applique sur toutes les couche mais ce n'est pas obligatoire,
#on peutt décider de ne l'appliquer que sur une partie des couches
#rate=0.1 : le pourcentage de neurone devant être désactivé, chaque neurone
#           a chaque étape a 10% de chance d'être désactivé.
#           La valeur peut évoluer de 0 à 1, faire evoluer la valeur pour supprimer l'overfitting
...
#PREDICTION SEULE
#----------------
""" Dans le même ordre que X qui à été catégorisé
Pays : France                 (0,0)
Score de crédit = 600         (600)
Genre : Male                  (0)
Age : 40                      (40)
Dans la banque depuis : 3 ans (3)
Balance : 60000               (60000)
Nb produit : 2                (2)
carte crédit : oui            (1)
Est actif : oui               (1)
Salaire : 50000               (50000)"""
#Création du tableau à une ligne avec les valeurs
ar = np.array([[0,0, 600, 0, 40, 3, 60000, 2, 1, 1, 50000]])
#Mise à l'échèlle des données 
ar = sc.transform(ar)
#réalisation de la prédiction
new_prediction = classifier.predict(ar)
#Convertion en bool

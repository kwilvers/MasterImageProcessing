        ...
        for (int rowIndex = padding; rowIndex < height - padding; rowIndex++)
        {
            ...
            for (int lineIndex = padding; lineIndex < width - padding; lineIndex++)
            {
                ...
                if (twoKernel)
                {
                    ...
                    var result = (byte)Math.Sqrt(Math.Pow(gray[0], 2) + Math.Pow(gray[1], 2));
                    resultBuffer[byteOffset + 1] = resultBuffer[byteOffset + 2] = resultBuffer[byteOffset];
                }
                else
                {
                    if (computeDirection)
                    ...
                    resultBuffer[byteOffset + 1] = resultBuffer[byteOffset + 2] = resultBuffer[byteOffset] = (byte)Max(gray);
                }
            }
        }
